...
export default class OrgModeTaskPlugin extends Plugin {
	...
	private registerCommands() {
		const commands = [
			...
			{ id: 'promote-task', name: 'Promote Task', method: (ops: EditorOperations) => this.promoteTask(ops), keyBindName: 'promoteTask' },
			{ id: 'demote-task', name: 'Demote Task', method: (ops: EditorOperations) => this.demoteTask(ops), keyBindName: 'demoteTask' }
		];
	...
	private toggleTaskState(ops: EditorOperations) {
		try {
			...
			if (match) {
				...
				const stateStartCh = indentation.length + listMarker.length + bullet.length + 1; // +1 for space after bullet
	...
	private getLineIndentationType(line: string): 'tabs' | 'spaces' | 'none' {
	...
	private getIndentationLevel(indentation: string): number {
		...
		for (const char of indentation) {
			if (char === '\t') {
				level += 2; // Assume a tab is equivalent to 2 spaces for level calculation
			} else if (char === ' ') {
				level += 1;
			}
		}
		...
		lines.forEach((line, index) => {
			...
			if (match?.groups) {
				...
				const taskItem: TaskItem = {
					line: index,
					indentation: indentation.length + listMarker.length,
					bullet,
					state,
					content: contentBeforeRatio.trim(),
					completedCount: completed ? parseInt(completed) : 0,
					totalCount: total ? parseInt(total) : 0,
					restOfLine,
					children: [],
	...
	private updateRatios(lines: string[]) {
		...
		for (let i = 0; i < updatedLines.length; i++) {
			...
			if (taskMatch?.groups) {
				const { indentation, listMarker = '', bullet, state, contentBeforeRatio, restOfLine = '' } = taskMatch.groups;
				const currentTaskFullIndentationLength = indentation.length + listMarker.length;

				// Look ahead for child tasks
				let children: any[] = [];
				let j = i + 1;
				while (j < updatedLines.length) {
					const childLine = updatedLines[j];
					const childMatch = childLine.match(TASK_REGEX);

					if (childMatch?.groups) {
						const childIndentation = childMatch.groups.indentation;
						const childListMarker = childMatch.groups.listMarker || '';
						const childState = childMatch.groups.state;

						const childFullIndentationLength = childIndentation.length + childListMarker.length;

						if (childFullIndentationLength > currentTaskFullIndentationLength) {
							children.push({
								line: j,
								indentation: childFullIndentationLength,
								state: childState,
							});
						} else {
							// Stop if indentation is not greater, meaning it's a sibling or parent task
							break;
						}
					} else if (childLine.trim().length > 0 && this.getIndentationLevel(childLine.match(/^(\s*)/)?.[1] || '') > this.getIndentationLevel(indentation)) {
						// If it's not a task but is indented more, it's considered part of the current task for ratio calculation purposes.
						// We don't need to add it to children array if it's not a task, but we need to continue iterating
					} else {
						break;
					}
					j++;
				...
				if (children.length > 0) {
					const completedChildren = children.filter(child => this.isTaskDone(child.state)).length;
					const totalChildren = children.length;

					// Reconstruct the line with the updated ratio
					updatedLines[i] = `${indentation}${listMarker}${bullet} ${state} ${contentBeforeRatio.trim()} [${completedChildren}/${totalChildren}]${restOfLine}`;
				}
			}
		}
	...
	private demoteTask = (ops: EditorOperations): void => {
		const cursor = ops.getCursor();
		const line = ops.getLine(cursor.line);
		const match = line.match(TASK_REGEX);
		...
		if (match?.groups) {
			...
			const indentationType = this.getLineIndentationType(line);
			let newIndentationString = '';
			let cursorChAdjustment = 0;

			if (indentationType === 'tabs') {
				newIndentationString = indentation + '\t';
				cursorChAdjustment = 1;
			} else {
				newIndentationString = indentation + '  ';
				cursorChAdjustment = 2;
			}

			const newIndentLevel = this.getIndentationLevel(newIndentationString);
			const newBullet = this.getBulletForLevel(newIndentLevel);

			const ratioPart = (completed && total) ? ` [${completed}/${total}]` : '';
			const newLine = `${newIndentationString}${listMarker}${newBullet} ${state} ${contentBeforeRatio}${ratioPart}${restOfLine}`;
			ops.setLine(cursor.line, newLine);

			ops.setCursor(cursor.line, cursor.ch + cursorChAdjustment);
			this.refreshUI(ops);
		}
	}

	private promoteTask = (ops: EditorOperations): void => {
		const cursor = ops.getCursor();
		const line = ops.getLine(cursor.line);
		const match = line.match(TASK_REGEX);
		...
		if (match?.groups) {
			...
			const indentationType = this.getLineIndentationType(line);
			let newIndentationString = indentation;
			let cursorChAdjustment = 0;

			if (indentationType === 'tabs') {
				if (indentation.length > 0) {
					newIndentationString = indentation.substring(1);
					cursorChAdjustment = -1;
				}
			} else if (indentationType === 'spaces') {
				if (indentation.length >= 2) {
					newIndentationString = indentation.substring(2);
					cursorChAdjustment = -2;
				} else {
					newIndentationString = '';
					cursorChAdjustment = -indentation.length;
				}
			} else {
				return; // No indentation, nothing to promote
			}

			// Only update if actual indentation changed
			if (newIndentationString === indentation) {
				return;
			}
			...
			ops.setCursor(cursor.line, Math.max(0, cursor.ch + cursorChAdjustment));
			this.refreshUI(ops);
		}
	}
	...
	private createTask = (ops: EditorOperations): void => {
		...
		const match = currentLine.match(TASK_REGEX);
		let currentIndentString = '';
		let indentLevel = 0;

		if (match?.groups) {
			currentIndentString = match.groups.indentation + (match.groups.listMarker || '');
			indentLevel = this.getIndentationLevel(match.groups.indentation);
		} else {
			const indentMatch = currentLine.match(/^(\s*)/);
			currentIndentString = indentMatch ? indentMatch[1] : '';
			indentLevel = this.getIndentationLevel(currentIndentString);
		}
		...
		const newTaskLine = `${currentIndentString}${bullet} ${this.settings.defaultTaskState} `;
